...
// Per-brand constants used by that brand's sequences. `name` + `accent` drive
// the branded HTML email layout (lib/email-html.js). `accent` is a placeholder
// brand color — tweak to match each brand's real palette.
const BRAND = {
  drfry: {
    site: 'https://drfry.nl',
  ...
  fahcel: {
    ...
    name: 'FahCel',
    accent: '#0E7490', // cool cold-chain teal
  },
};

// The brand theme for a sequence (used to render its HTML email layout).
export function brandTheme(brandKey) {
  return BRAND[brandKey] || null;
}
...
// Render a step into { stepId, subject, text, html } for a lead. `text` stays
// the plain-text source/fallback; `html` is the branded layout for that
// sequence's brand (undefined if the sequence has no brand theme).
export function renderStep(seq, stepIndex, lead, backendBase) {
  const step = seq.steps[stepIndex];
  const text = step.body(lead, backendBase);
  const theme = BRAND[seq.brand];
  const html = theme ? renderEmailHtml(text, theme) : undefined;
  return { stepId: step.id, subject: step.subject(lead), text, html };
}
