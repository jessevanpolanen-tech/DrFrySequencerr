...
// ── Leads ──────────────────────────────────────────────────────────
// `tenant` keeps each brand's leads separate on the SHARED database. It is
// written on INSERT only — an existing lead never switches tenant.
//
// TENANT MUST be set per deployment (e.g. `drfry` | `fahcel`). There is
// deliberately NO brand default: a silent default is exactly what mislabeled
// leads across brands before, so an unset TENANT fails loudly instead of
// quietly tagging every new lead with the wrong brand.
export const DEFAULT_TENANT = (() => {
  const t = (process.env.TENANT || '').trim().toLowerCase();
  if (!t) throw new Error('TENANT env var is required (e.g. drfry | fahcel)');
  return t;
})();
